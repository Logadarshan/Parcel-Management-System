...
class goodsListController extends Controller
{
    ...
    public function update()
    {
        ...
        $data = DB::table('parcels')
            ...
            ->update(['parcels.p_id' => $p_id, 'parcel_details.p_id' => $p_id, 'parcel_details.collect_date' => $collect_date, 'parcels.p_status' => $p_status, 'parcel_details.ready_date' => $ready_date, 'parcels.std_status' => $std_status, 'parcel_details.student_receive_date' => $student_receive_date]);
        return redirect('/Good-list');
    }
    ...
    public function report()
    {
        ...
        $months  =  DB::table('parcels')
            ->join('parcel_details', 'parcels.p_id', '=', 'parcel_details.p_id')
            ->join('users', 'parcels.u_id', '=', 'users.u_id')
            ->select('users.u_id', 'receive_date')->select('parcel_details.*', 'parcels.*', 'users.*')->where('users.u_id', '=', $currentid)->where('parcels.std_status', '=', 'Received')->where('receive_date', '>', '2020-01-01')->get()->groupBy(function ($date) {
